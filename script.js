  ...
  recipes.forEach((item) => {
    ...
    card.innerHTML = `
      <div class="container__card container__card--left">
        ...
        <img src="http://194.87.214.100/${item.picture}" class="recipe__image" width="200" height="300">
      </div>
...
const getFilter = () => {
  ...
  search.addEventListener("keyup", (event) => {
    ...
    cards.forEach((item) => {
      const ingredients = Array.from(item.querySelectorAll(".ingredient__name"))
        .map((item) => item.textContent.toLowerCase())
        .join(", ");
      item
        .querySelector(".recipe__name")
        .textContent.toLowerCase()
        .includes(word) || ingredients.includes(word)
